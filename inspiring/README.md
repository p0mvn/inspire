...
- **Stable Rust**: pinned by the workspace [`rust-toolchain.toml`](../rust-toolchain.toml)
  to `1.89.0`. `rustup` will install the right toolchain when you
  `cd inspiring && cargo build` (or build from the repo root with `cargo build -p inspiring`).
...
```
ipir-sp/                        # workspace root — open this folder in the IDE for rust-analyzer
|-- Cargo.toml
|-- rust-toolchain.toml
`-- inspiring/
    |-- Cargo.toml
...
```bash
cd inspiring
cargo build --release           # stable 1.89.0 via workspace rust-toolchain.toml (repo root)
cargo test
